...
float4 LightPixelShader(PixelInputType input) : SV_TARGET
{
	...
	textureColour = textures[0].Sample(TrilinearWrap, input.UV);
	...
	if (useAlphaMap)
	{
		float4 alphaColour = textures[1].Sample(TrilinearWrap, input.UV);
...
float4 ModelRefractionPS(PixelInputType input) : SV_TARGET
{
	...
	float3 sceneColour = LightPixelShader(input).rgb;
